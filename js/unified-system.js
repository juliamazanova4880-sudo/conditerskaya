...
class UnifiedSystem {
  ...
  // 🔥 ОСНОВНОЙ МЕТОД: РЕНДЕР КАТАЛОГА С AJAX
  renderCatalog() {
  ...
  // Fallback: рендер через JavaScript (как раньше)
  ...
  // Фильтрация товаров (всегда через JS)
  initFilters() {
  ...
  filterProducts(filter) {
    ...
    // ВСЕГДА рендерим через JS (AJAX не нужен при фильтрации)
    this.renderFilteredFallback(filtered);
  }

  renderFilteredFallback(products) {
    const container = document.getElementById('catalog-grid');
    if (!container) return;
  ...
  // ========== СИСТЕМА АВТОРИЗАЦИИ ========== (без изменений)
  loadUsers() {
    try {
  ...
  // ========== АДМИН-ПАНЕЛЬ ========== (стилизована под сайт)
  showAdminPanel() {
    if (!this.isAdmin) {
    ...
    let content = `
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
        <button class="admin-btn primary" data-action="catalog">📦 Управление каталогом</button>
        <button class="admin-btn info" data-action="users">👥 Управление пользователями</button>
        <button class="admin-btn warning" data-action="system">📊 Системная информация</button>
        <button class="admin-btn secondary" data-action="deleted">🗑️ Скрытые товары (${this.deletedProductIds.length})</button>
      </div>
  ...
  showCatalogManager() {
    ...
    content.innerHTML = `
      ...
      <div style="margin-bottom: 20px; display: flex; gap: 10px;">
        <button class="admin-btn primary" data-action="add-product">➕ Добавить товар</button>
      </div>
        ...
        ${this.catalog.map(product => `
          <div class="product-card" style="background: #f8f8f8; padding: 15px; border-radius: 8px;">
            ...
            <div style="font-size: 12px; color: #888; margin-bottom: 15px;">
              Категория: ${product.category} | ${product.available ? '✅ В наличии' : '❌ Нет в наличии'}
            </div>
            <div style="display: flex; gap: 10px;">
              <button class="admin-btn small edit" data-id="${product.id}">✏️ Редактировать</button>
              <button class="admin-btn small ${product.isDefault ? 'archive' : 'delete'}" data-id="${product.id}">${product.isDefault ? '📦 Скрыть' : '🗑️ Удалить'}</button>
            </div>
          </div>
        `).join('')}
      </div>
  ...
  showDeletedProducts() {
        ...
        if (defaultProduct) {
          html += `
            <div style="background: #f8f8f8; padding: 15px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center;">
              ...
              <button class="admin-btn small success" data-id="${id}">🔄 Восстановить</button>
            </div>
          `;
        }
      });
    ...
    html += `
      <div style="margin-top: 20px;">
        <button class="admin-btn secondary" data-action="back-to-catalog">← Назад к каталогу</button>
      </div>
    `;
  ...
  showAddProductForm() {
    ...
    content.innerHTML = `
      ...
      <form id="add-product-form-admin" style="margin-top: 20px;">
        <div style="margin-bottom: 15px;"><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Название товара</label>
          <input type="text" id="new-product-name" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px;">
        </div>
        <div style="margin-bottom: 15px;"><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Описание</label>
          <textarea id="new-product-description" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; min-height: 80px;"></textarea>
        </div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
          <div><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Цена (${SYSTEM_CONFIG.CURRENCY})</label>
            <input type="number" id="new-product-price" min="0" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px;">
          </div>
          <div><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Категория</label>
            <select id="new-product-category" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px;">
        ...
        <div style="margin-bottom: 20px;"><label><input type="checkbox" id="new-product-available" checked> Товар доступен для заказа</label></div>
        <div style="display: flex; gap: 15px;">
          <button type="button" class="admin-btn secondary" data-action="cancel-add">← Отмена</button>
          <button type="submit" class="admin-btn primary">Добавить товар</button>
        </div>
      </form>
    `;
  ...
  addProduct(product) {
    const newProduct = { ...product, id: Date.now(), isDefault: false };
  ...
  editProduct(id) {
    ...
    content.innerHTML = `
      ...
      <form id="edit-product-form-admin" style="margin-top: 20px;">
        ...
        <div style="margin-bottom: 15px;"><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Название товара</label>
          <input type="text" id="edit-product-name" value="${product.name}" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px;">
        </div>
        <div style="margin-bottom: 15px;"><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Описание</label>
          <textarea id="edit-product-description" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; min-height: 80px;">${product.description}</textarea>
        </div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
          <div><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Цена (${SYSTEM_CONFIG.CURRENCY})</label>
            <input type="number" id="edit-product-price" value="${product.price}" min="0" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px;">
          </div>
          <div><label style="display: block; margin-bottom: 8px; color: #432719; font-weight: 500;">Категория</label>
            <select id="edit-product-category" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px;">
        ...
        <div style="margin-bottom: 20px;"><label><input type="checkbox" id="edit-product-available" ${product.available ? 'checked' : ''}> Товар доступен для заказа</label></div>
        <div style="display: flex; gap: 15px;">
          <button type="button" class="admin-btn secondary" data-action="cancel-edit">← Отмена</button>
          <button type="submit" class="admin-btn primary">Сохранить изменения</button>
        </div>
      </form>
    `;
      ...
      <div style="margin-top: 20px;">
        ${this.users.map(user => `
          <div style="background: #f8f8f8; padding: 15px; border-radius: 8px; margin-bottom: 10px;">
            ...
            <div style="color: #666; font-size: 14px; margin-top: 5px;">${user.email}</div>
            <div style="font-size: 11px; color: #999; margin-top: 3px;">${new Date(user.registration_date).toLocaleDateString('ru-RU')}</div>
          </div>
        `).join('')}
      </div>
    `;
  ...
  showSystemInfo() {
    ...
    content.innerHTML = `
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 20px;">
        <div style="background: #e8f5e9; padding: 15px; border-radius: 8px;"><div style="font-size: 24px; color: #4CAF50;">${this.catalog.length}</div><div style="color: #666; font-size: 14px;">Товаров в каталоге</div></div>
        <div style="background: #e3f2fd; padding: 15px; border-radius: 8px;"><div style="font-size: 24px; color: #2196F3;">${this.users.length}</div><div style="color: #666; font-size: 14px;">Зарегистрированных пользователей</div></div>
        <div style="background: #fff3e0; padding: 15px; border-radius: 8px;"><div style="font-size: 24px; color: #ff9800;">${orders.length}</div><div style="color: #666; font-size: 14px;">Оформленных заказов</div></div>
        <div style="background: #ffebee; padding: 15px; border-radius: 8px;"><div style="font-size: 24px; color: #f44336;">${cartItems.length}</div><div style="color: #666; font-size: 14px;">Товаров в активных корзинах</div></div>
      </div>
  ...
  // ========== УВЕДОМЛЕНИЯ НА VUE.JS (уже есть в вашем коде — оставляем как есть) ==========
  ...
  animateAddToCart(button, product) {
    ...
    if (cartIcon) {
      ...
      const animation = flyIcon.animate([
        { left: flyIcon.style.left, top: flyIcon.style.top, fontSize: '24px', opacity: 1 },
        { left: (cartRect.left + 10) + 'px', top: (cartRect.top + 10) + 'px', fontSize: '12px', opacity: 0.7 }
  ...
  addStyles() {
    ...
    style.textContent = `
      ...
      .system-toast { animation: slideIn 0.3s ease; }
      /* Стили для корзины */
      .cart-icon.bounce { animation: bounce 0.5s; }
      @keyframes bounce { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.2); } }
      .flying-icon { z-index: 10000; pointer-events: none; }
      ...
      .admin-btn.primary { background: #ffafbc; color: #432719; }
      .admin-btn.secondary { background: #f5e6e8; color: #432719; }
      .admin-btn.success { background: #c8e6c9; color: #2e7d32; }
      .admin-btn.warning { background: #ffe0b2; color: #e65100; }
      .admin-btn.info { background: #e3f2fd; color: #0d47a1; }
      .admin-btn.delete, .admin-btn.archive { background: #ffcdd2; color: #b71c1c; }
      .admin-btn.small { padding: 8px 16px; font-size: 13px; }
      .admin-btn:hover {
      ...
      .cart-item-name { font-weight: 600; color: #432719; margin-bottom: 5px; }
